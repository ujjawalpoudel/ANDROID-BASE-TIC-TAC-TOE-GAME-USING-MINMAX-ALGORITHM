...
public class AI {
    ...
    AI(String marker) //Constructor Call
    {
        this.myMark = marker;
    ...
    private int[] minmax(int depth, String player) {
        ...
        if (nextMoves.isEmpty())
        {
           ...
           return new int[] { score, bestRow, bestCol };
            ...
            for (int[] move : nextMoves)
            {
                ...
                if (player == myMark)
                { // mySeed (computer) is maximizing
                    // player
                    score = minmax(depth + 1, oppMark)[0];
                    if (score > max)
                    {
                        max = score;
                        bestRow = move[0];
        ...
        return new int[] { (player == myMark) ? max : min, bestRow, bestCol };
    }
     ...
     *  -100, -10, -1 for EACH 3-, 2-, 1-in-a-line for opponent. 0 otherwise */
